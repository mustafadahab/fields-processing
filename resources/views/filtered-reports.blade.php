...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            @if($filtered_by != 'Reset')
                <h2 align="center">({{ $filtered_by  }})</h2>
            @endif
    ...
    <div class="table-responsive-md">
        <table class="table">
            ...
            @foreach ($processed_fields as $processed_field)
                <tr>
                    <th scope="row">{{ $processed_field->processed_fields_id }}</th>
                    <td>{{ $processed_field->name }}</td>
                    <td>{{ $processed_field->type_of_crops }}</td>
                    <td>{{ $processed_field->added_on }}</td>
                    <td>{{ $processed_field->tractor_name }}</td>
                    <td>
